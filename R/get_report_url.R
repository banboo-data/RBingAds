...
#'
#' @description get_report_url request the download URL related to the report id generated by get_report_id().
#'
...
#'
#' @export
get_download_url <- function(bing_auth, customer_id, account_id, report_id){
  ...
  body_xml <- '<PollGenerateReportRequest xmlns="https://bingads.microsoft.com/Reporting/v13"><ReportRequestId i:nil="false">%s</ReportRequestId></PollGenerateReportRequest>'
  ...
  body <- sprintf(header,
                  ...
                  body_xml)
  header_fields =  c(Accept = "text/xml", Accept = "multipart/*", 'Content-Type' = "text/xml;charset=utf-8", SOAPAction = soap_action)
  h = basicTextGatherer()
  curlPerform(url = url, httpheader = header_fields, postfields = body, writefunction = h$update)
